...
fp_virhost <- here(dpath, "Host_prediction_to_genome_m90.csv")
...
df_tpm <- read_abundance(fp_vir_tpm, anno_viral, smeta$sample_id)
...
counts2rpkm <- function(df_counts, seq_len) {
  df_rpkm <- sweep(df_counts * 10^9 / 2, 1, seq_len, FUN = '/')
  return(df_rpkm)
}

counts2tpm <- function(df_counts, seq_len, ceiling=FALSE) {
  df_rpkm <- sweep(df_counts * 10^9 / 2, 1, seq_len, FUN = '/')
  df_tpm <- sweep(df_rpkm, 2, colSums(df_rpkm), FUN = '/') * 10^6
  if (ceiling) {
...
tse <- TreeSummarizedExperiment(assays = SimpleList(tpm = as.matrix(mx_tpm)),
                                colData = smeta,
...
```{r}
tse1 <- tse_vc
tse1 <- transformCounts(tse1, method = "relabundance", abund_values = "tpm")

idx_richness <- c("chao1", "ace")
tse1 <- estimateRichness(tse1, abund_values = "tpm",
                        index = idx_richness,
                        name = paste0("richness_", idx_richness))

idx_diversity <- c("coverage", "fisher", "shannon")
tse1 <- estimateDiversity(tse1, abund_values = "tpm",
                        index = idx_diversity,
                        name = paste0("diversity_", idx_diversity))

idx_evenness <- c("pielou", "camargo", "simpson_evenness", "evar", "bulla")
tse1 <- estimateEvenness(tse1, abund_values = "tpm",
                        index = idx_evenness,
...
p1 <- plot_beta_diversity(tse1, name = "PCoA", method = "bray", exprs_values = "relabundance", ntop = nrow(tse))
p2 <- plot_beta_diversity(tse1, name = "MDS_euclidean", method = "euclidean", exprs_values = "relabundance", ntop = nrow(tse))
p3 <- plot_beta_diversity(tse1, name = "NMDS_BC", method = "bray", exprs_values = "relabundance", ntop = nrow(tse), NMDS = TRUE)
p4 <- plot_beta_diversity(tse1, name = "NMDS_euclidean", method = "euclidean", exprs_values = "relabundance", ntop = nrow(tse1), NMDS = TRUE)
...
rank_sel <- "Family"
tse_tmp <- tse_hd
tse_tmp <- relAbundanceCounts(tse_tmp, abund_values = "tpm")
...
ggsave("fig1_family_abundance.pdf")
...
ggsave("fig2_diversity.pdf", plot = fig2, width = 14)
```
...
pheatmap(df4heatmap, scale = "column", width = 10, fontsize = 9, 
         cluster_rows = T,
         cluster_cols = F,
